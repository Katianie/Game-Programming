...
/*
	...
	be added once.
*/
/*void GameText::addText(LPCWSTR textToAdd, 
					   int initX, 
					   int initY, 
					   int initWidth, 
					   int initHeight)
*/
void GameText::addText(wchar_t *textToAdd,
					   int initX,
					   int initY,
					   int initWidth,
					   int initHeight)
...
*/
void GameText::changeTextOnly(wchar_t *textToAdd, int index)
{
	TextToDraw *textToChange = textToDraw->at(index);
	wchar_t *oldText = textToChange->getText();
	delete [] oldText;
	textToChange->setText(textToAdd);
}
