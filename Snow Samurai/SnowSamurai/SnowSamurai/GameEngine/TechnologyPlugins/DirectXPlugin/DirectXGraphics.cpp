...
HRESULT DirectXGraphics::createDirectXDeviceAndSpriteHandler()
{
	...
	if (SUCCEEDED(result))
	{
		game->writeOutput(L"SUCCEEDED");

		this->game->writeOutput(L"\nD3DXCreateSprite(): ");
...
void DirectXGraphics::findAlternativeDisplayMode(	vector<D3DDISPLAYMODE*> *displayModes,
													...
													int &screenWidthToSet,
													int &screenHeightToSet)
{
	...
	{
		D3DDISPLAYMODE *testMode = (*iterator);
		if ((testMode->Width == DEFAULT_SCREEN_WIDTH)
			&& (testMode->Height == DEFAULT_SCREEN_HEIGHT))
		{
			screenWidthToSet = DEFAULT_SCREEN_WIDTH;
			screenHeightToSet = DEFAULT_SCREEN_HEIGHT;
	...
	while (iterator != displayModes->end())
	{
		...
		{
			formatToSet = testMode->Format;
			screenWidthToSet = testMode->Width;
			screenHeightToSet = testMode->Height;
...
void DirectXGraphics::initGraphics(HWND hWnd, bool isFullscreen)
{
							...
							DEFAULT_SCREEN_HEIGHT))
	{
		...
		screenWidthToUse = DEFAULT_SCREEN_WIDTH;
		screenHeightToUse = DEFAULT_SCREEN_HEIGHT;
	}
	else
	{
...
*/
void DirectXGraphics::shutdownGraphics()
{
	if (textFont)
	{
		textFont->Release();
